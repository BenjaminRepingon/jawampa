...
public class QueueingConnectionController implements IConnectionController
{
	static class QueuedMessage
	{
		...
		QueuedMessage( WampMessage message, IWampConnectionPromise<Void> promise )
		{
	...
	private final ICompletionCallback<Void> messageSentHandler = new ICompletionCallback<Void>()
	{
		@Override
	...
	private final ICompletionCallback<Void> connectionClosedHandler = new ICompletionCallback<Void>()
	{
		@Override
		public void onCompletion( final IWampConnectionFuture<Void> future )
		{
	 ...
	 */
	private final WampConnectionPromise<Void> messageSentPromise = new WampConnectionPromise<Void>( messageSentHandler, null );

	/**
	 * Promise that will be fulfilled when the connection was closed
	 * and the close was acknowledged by the underlying connection.
	 */
	private final WampConnectionPromise<Void> connectionClosedPromise = new WampConnectionPromise<Void>( connectionClosedHandler, null );
	 ...
	 */
	private IWampConnection connection;
	 ...
	 * Queued messages
	 */
	private Deque<QueuedMessage>         queuedMessages = new ArrayDeque<QueuedMessage>();

	/**
	 * Holds the promise that will be fulfilled when the connection was closed
	 */
	private IWampConnectionPromise<Void> queuedClose    = null;
	...
	public QueueingConnectionController( ScheduledExecutorService scheduler, IWampConnectionListener connectionListener )
	{
