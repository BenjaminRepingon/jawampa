...
{
	private static final Pattern LOOSE_URI          = Pattern.compile( "^([^\\s\\.#]+\\.)*([^\\s\\.#]+)$" );
	private static final Pattern LOOSE_URI_PREFIX   = Pattern.compile( "^([^\\s\\.#]+\\.)*([^\\s\\.#]+)?$" );
	private static final Pattern LOOSE_URI_WILDCARD = Pattern.compile( "^(([^\\s\\.#]+\\.)|\\.)*([^\\s\\.#]+)?$" );

	private static final Pattern STRICT_URI          = Pattern.compile( "^([0-9a-z_]+\\.)*([0-9a-z_]+)$" );
	private static final Pattern STRICT_URI_PREFIX   = Pattern.compile( "^([0-9a-z_]+\\.)*([0-9a-z_]+)?$" );
	private static final Pattern STRICT_URI_WILDCARD = Pattern.compile( "^(([0-9a-z_]+\\.)|\\.)*([0-9a-z_]+)?$" );
