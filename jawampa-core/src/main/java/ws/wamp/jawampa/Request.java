...
public class Request
{
	...
	private final SessionEstablishedState session;
	...
	public void reply( ArrayNode arguments, ObjectNode keywordArguments )
	{
		...
		stateController.tryScheduleAction( new Runnable()
		{
			...
			public void run()
			{
				if ( stateController.currentState() != session )
					return;
				session.connectionController().sendMessage( msg, IWampConnectionPromise.LogError );
			}
		} );
	 ...
	 * @param arguments        The positional arguments to sent in the response
	 */
	public void reply( Object... arguments )
	{
		reply( ArgArrayBuilder.buildArgumentsArray(
				stateController.clientConfig().objectMapper(), arguments ), null );
	}
